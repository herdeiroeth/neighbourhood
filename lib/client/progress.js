...
export class ProgressBar {
  constructor(totalBytes, initialBytes = 0) {
    this.totalBytes = totalBytes;
    this.received = initialBytes;
    this.transferred = 0;
  ...
  render() {
    ...
    const pct = this.totalBytes > 0
      ? Math.min(1, Math.max(0, this.received / this.totalBytes))
      : 0;
  ...
  finish() {
    ...
    const avgSpeed = elapsed > 0 ? this.transferred / elapsed : 0;
