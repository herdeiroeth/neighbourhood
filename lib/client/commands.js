...
// Track current download state for SIGINT handling
let currentWriteStream = null;

function onSigint() {
  const ws = currentWriteStream;
  if (ws) ws.end();
    ...
    res.on('data', (chunk) => {
      ...
      while (buffer.length > 0) {
        ...
        if (state === 'data') {
          ...
          if (toWrite > 0) {
            currentWriteStream.write(buffer.subarray(0, toWrite));
...
// Register shared SIGINT handler
process.on('SIGINT', onSigint);
