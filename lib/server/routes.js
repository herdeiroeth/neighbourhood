...
function isWithinRoot(rootDir, targetPath) {
  // Windows realpath variants can differ only by the `\\?\` namespace
  // prefix. Compare both paths in namespaced form so the same directory is
  // not rejected as an unrelated absolute path.
...
async function safePath(rootDir, requestedPath) {
  ...
  // Use the same realpath implementation as the server root. On Windows,
  // mixing the sync and promise variants can produce different device-path
  // representations for the same file.
